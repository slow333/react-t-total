...
function CssMedium() {
  ...
  content: "•"; /* 여기에 머리글을 지정하는 뭐든지 넣을 수 있음 */
      ...
      <pre>{`p::before {
  content: "Before: ";  // 요소의 내용 앞에 추가할 콘텐츠
}`}</pre>

      <h2>float</h2>
      <p>The float property is used for positioning and formatting content, 
        e.g., allowing text to wrap around an image.</p>
      <pre>{`img {
  float: left; // right
  margin: 0 15px 15px 0;
}`}</pre>
      <h3>clear</h3>
      <p>The clear property is used to control the behavior of floating elements.</p>
      <pre>{`div {
  clear: both;
}`}</pre>
      <h3>float</h3>
      <h3>자신이 포함된 container의 높이를 넘어섰을 때 이를 보정하기 위해서</h3>
      <pre>상위 컴포넌트에 displayL flow-root;를 적용하면
  float를 사용한 자식 컴포넌트의 높이를 보정할 수 있습니다.</pre> 
      <p>이렇게 하면 float를 사용한 자식 컴포넌트의 높이를 보정할 수 있습니다.</p>
  ...
  clear: both;
    ...
    <h2>column</h2>
      ...
      <div className="columns">
        ...
        <div className="quote">&#8220;큰 따움표 표시&#8221; <span className='nowrap'>&#8212;내용 분리안됨</span></div>
        <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Nesciunt, illum.</p>
        <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Nesciunt, illum.</p>
      </div>
      ...
      <p>자신의 parent 중 ralative에 상대적 위치를 갖음, relative가 없으면 window에 따라감</p>
    </>
  )
}
