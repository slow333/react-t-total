...
function SpringAccessControll() {
  return (
    <div>
      <h1>Restricting user only their own data</h1>
      <h3>자신의 데이터에만 접근 가능하게 하기; 이거 없으면 다른 사용자 정보에 접근 가능</h3>
      <h3>자신의 데이터만 지우고, 변경하고, 조회하는게 가능</h3>
  <pre>{`// jwt를 다 받은 이후에 이 토큰을 이용해서 접근하려 할때
...
token =▷ AuthenticationProvider -▷ UserController -▷ UserService -▷ UserRepository
  ...
  <h3>jwt에서 userId를 얻기 위한 설정</h3>
<pre>{`@DeleteMapping("/userId")
public Result deleteUser(
    @PathVariable Integer userId, 
    Authentication authentication){
  // 여기에 break point 걸고 delete 실행하면 인증이 보임
  this.userService.delete(userId); 
}
// JwtProvider에 claim 추가해서 jwt authentication에서 id를 받아 올수 있게 함
JwtClaimSet.builder()
  .claim("userId", 
    ((MyPrincipal)authentication.getPrincipal()).getHoUser().getId())`}
  </pre>
      <h3>AuthorizationManager 설정</h3>
  <pre>{`@Component
public class UserRequestAuthorizationManager implements AuthorizationManager[RequestAuthorizationContext] {
    ...
    private static final UriTemplate URI = 
        new UriTemplate("/api/v1/users/{userId}");
    ...
    public AuthorizationDecision check(
      ...
      Map[String, String] uriVariables = 
          URI.match(context.getRequest().getRequestURI());
      String userIdFromUri = uriVariables.get("userId");

      // jwt auth에서 userId를 추출
      Authentication authentication = authenticationSupplier.get();
      String userIdFromJwt = ((Jwt) authentication.getPrincipal())
      ...
      // 2개의 아이디를 비교
      boolean userIdsMatch = userIdFromUri != null 
          && userIdFromUri.equals(userIdFromJwt);
      return new AuthorizationDecision(hasAdminRole 
          || (hasUserRole && userIdsMatch));
    }
}`}
  </pre>
      <h3>Security 설정</h3>
...
private final UserRequestAuthorizationManager userRequestAuthorizationManager;
  ...
  .requestMatchers(HttpMethod.GET, "/users")
      .hasRole("ROLE_admin") // 모든 사용자 조회는 admin 만
  .requestMatchers(HttpMethod.GET, "/users/**")
      .access(userRequestAuthorizationManager) // boolean으로 true면 허가`}
  </pre>
      <h3>integration test 수정 </h3>
  <pre>{`admin 일 때 user 일 때에 따라서 수정
  jwt token 생성을 admin, user 일 때에 맞추어 생성하고 넣어 줘야함
  접속되는거 안되는 거 등등`}</pre>
      <h3>update service 에서 admin 일 때와 user 일 때를 구분; user는 username 정도만 변경 가능</h3>
      <h4>password 변경은 별도로 다룸..</h4>
  <pre>{`
...
export default SpringAccessControll
