...
<pre>{`
  ...
  public class Result {
    ...
    public class StatusCode {
        // http error code를 정의: 기본 HttpStatusCode을 
        // 사용해도 되나 개발자에 따라 요구가 다를 수 있으므로 정의해서 사용
    ...
    <pre>{`
  Optional[Artifact] findByIdAndNameAndDescription(
  ...
  // @Query 옵션에서 nativeQuery = true를 줘야지만... 
  //   DB에서 쿼리문을 작성하는 방식으로 작성할 수 있음
  ...
  User findByLastnameOrFirstname(
      String lastname, String firstname); //이렇게도 사용 가능하다.
