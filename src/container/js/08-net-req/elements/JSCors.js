...
function JSCors() {
  return (
    <div>
      <div className="for-space" id='js-cors'></div>
      <h1>Cors</h1>
      <h3>종류 : 안전한 요청(GET, POST), 그외 요청</h3>

      <h3>CORS와 안전한 요청</h3>
      <h4>크로스 오리진 요청을 보낼 경우 브라우저는 항상 Origin이라는 헤더를 요청에 추가</h4>
    <pre>GET /request
    Host: anywhere.com
    ...
    ...</pre>
      ...
      <h3>안전하지 않은 요청</h3>
      <h4>GET, POST 뿐만 아니라 PATCH, DELETE 등 어떤 메서드도 사용할 수 있습니다.</h4>
      ...
      <h4>안전하지 않은 요청을 허용하기로 협의하였다면 서버는 본문이 비어있고 상태 코드가 200인 응답을 다음과 같은 헤더와 함께 브라우저로 보냅니다.</h4>
      <h5>Access-Control-Allow-Origin – *이나 요청을 보낸 오리진 이어야 합니다(예: https://javascript.info)</h5>
      <h5>Access-Control-Allow-Methods – 허용된 메서드 정보가 담겨있습니다.</h5>
      <h5>Access-Control-Allow-Headers – 허용된 헤더 목록이 담겨있습니다.</h5>
      <h5>Access-Control-Max-Age – 퍼미션 체크 여부를 몇 초간 캐싱해 놓을지를 명시합니다. 이렇게 퍼미션 정보를 캐싱해 놓으면 브라우저는 일정 기간 동안 preflight 요청을 생략하고 안전하지 않은 요청을 보낼 수 있습니다.</h5>
      <h3>진행 절차</h3>
