...
function JSFetch() {
  return (
    <div>
      <div className="for-space" id='js-fetch-basic'></div>
      <h1>fetch</h1>
  ...
  <h3>response type</h3>
  <h4>response.text(), response.json(), response.formData(), response.blob(), response.arrayBuffer()</h4>
  <h4>json()</h4>
<pre>{`fetch(url)
...
setTimeout(() => { // 3초 후 이미지를 숨깁니다.
  img.parentNode.childNode(img).remove();
  URL.revokeObjectURL(img.src);
}, 3000);`}</pre>

    <h3>응답헤더</h3>
    <h4>응답 헤더는 response.headers에 맵과 유사한 형태로 저장됩니다.</h4>
    <h4>맵은 아닙니다. 하지만 맵과 유사한 메서드를 지원하죠. 이 메서드들을 사용하면 헤더 일부만 추출하거나 헤더 전체를 순회할 수 있습니다.</h4>
<pre>{`fetch('https://api.github.com/repos/javascript-tutorial
  ...
  headers: {
    ...
    <h3>POST request</h3>
    <h4>GET 이외의 요청을 보내려면 추가 옵션을 사용해야함</h4>
    <h5>method : HTTP method(ex; POST);</h5>
    <h5>body : 문자열(JSON), formData, blob 중 하나이여야 함</h5>
...
<pre>{`canvasElem.onmousemove = function(e) {
