function JSPromise() {
  ...
  return (
    <div>
      <div className="for-space" id='js-promise-basic'></div>
...
.catch(failureCallback);   `}</pre>
    <p>Promise는 이벤트 리스너와 유사하지만 몇가지 다른점이 있습니다.</p>
    <ul>
      ...
      <h4>resolve(value) — 일이 성공적으로 끝난 경우 그 결과를 나타내는 value와 함께 호출</h4>
      <h4>reject(error) — 에러 발생 시 에러 객체를 나타내는 error와 함께 호출</h4>
...
promise.then(
  ...
  return new Promise(function(resolve, reject) {
    ...
    script.onerror = () => reject(new Error( 백틱 {src}를 불러오는 도중에 에러가 발생함));
...
promise.then(
  script => alert(백틱 {script.src}을 불러왔습니다!),
  error => alert(백틱 Error: {error.message})
);
