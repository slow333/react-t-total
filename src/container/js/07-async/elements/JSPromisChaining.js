...
function JSPromisChaining() {
  return (
    <div>
      <div className="for-space" id='js-promise-chaining'></div>
...
return fetch(url)
  ...
  .catch((e) => {
    console.log(
      백틱 There has been a problem with your fetch operation for resource "{url}": +
        e.message,
    );
  })
  .finally(() => {
  ...
  .then(user => fetch(백틱 https://api.github.com/users/{user.name}))
  .then(response => response.json())
  ...
  // 3초 후 동작함
  .then(githubUser => alert(백틱 {githubUser.name}의 이미지를 성공적으로 출력하였습니다.))
...
function loadGithubUser(name) {
  return fetch(백틱 https://api.github.com/users/{name})
    .then(response => response.json());
}
...
loadJson('/article/promise-chaining/user.json')
  ...
  .then(showAvatar)
  .then(githubUser => alert(백틱 Finished showing {githubUser.name}))
  ...
  blabla(); // 존재하지 않는 함수
    ...
    <h3>처리되지 못한 거부</h3>
    <h4>자바스크립트 엔진은 프라미스 거부를 추적하다가 위와 같은 상황이 발생하면
      전역 에러를 생성합니다. 콘솔창을 열고 예시를 실행하면 전역 에러를 확인할 수 있습니다.</h4>
    <h4>브라우저 환경에선 이런 에러를 unhandledrejection 이벤트로 처리할 수 있습니다.</h4>
...
export default JSPromisChaining
