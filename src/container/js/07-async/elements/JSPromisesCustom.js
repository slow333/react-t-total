...
function JSPromisesCustom() {
  return (
    <div>
      <div className="for-space" id='js-promise-custom'></div>
      <h1>Custome Promises</h1>
    ...
    setTimeout(function () {
      ...
      <p>resolve() 와reject() 는 Promise의 fulful / reject일때의 일을 수행하기 위해 호출한 함수입니다. 이번의 경우 Promise는 "Success!"문자와 함께 fulfill 됐습니다.<br/> 
      따라서 이 Promise를 호출할 때, 그 끝에 .then() 블럭을 사용하면 "Success!" 문자열이 전달될 것입니다. 아래 코드는 간단한 alert메시지를 출력하는 방법 입니다. :</p>
      <pre>{`timeoutPromise.then((message) => { alert(message); });
      OR :: timeoutPromise.then(alert); `}</pre>
...
export default JSPromisesCustom
