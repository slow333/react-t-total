...
function JSBasic() {
  ...
  return (
    <div>
      ...
      <h3>코드를 클래스와 모듈을 사용해 구성한다면 "use strict"를 생략해도 됩니다.</h3>
      ...
      */
      ...
      <h2>바벨</h2>
      <h3>바벨(Babel)은 트랜스파일러(transpiler)로, 
        모던 자바스크립트 코드를 구 표준을 준수하는 코드로 바꿔줍니다.</h3>
        <h1>자료형</h1>
      <h2>숫자형</h2>
    ...
    "age" in user; // true`}</pre>
      ...
      <h1>형변환</h1>
      <h2>문자형 변환 :String(value); num + '';</h2>
      <p>""로 더하기 하면 문자료 변환됨</p>
      <h2>숫자로 변환 : Number('239');</h2>
  ...
  " \t \n" - 2 = -2 // (7)`}</pre>
      ...
      <h2>boolean</h2>
      <h3>0, null, undefined, NaN, ""  // false</h3>
  ...
  alert(a == b); // true!`}</pre>
    ...
    0 === false; // false</pre>
      ...
      <h2>nullish 연산자 : ??</h2>
      <h3>??를 사용하면 값이 할당된 변수를 빠르게 찾을 수 있음</h3>
      <h3>height = height ?? 100; // height가 undefined 또는 null 이면 100을 할당</h3>
      <h3>height가 0이면 0이 할당됨. </h3>


      <h2>string indexOf; str.indexOf('id', 2); 2부터 찾음</h2>
      <h3>str : includes, startsWith, endsWith ; 불린 반환</h3>
      <p>비교적 근래에 나온 메서드인 str.includes(substr, pos)는 str에 부분 문자열 substr이
        있는지에 따라 true나 false를 반환합니다.</p>
      <h3>문자열 추출: str.slice(start[,end]); // start 부터 end까지(end미포함), 음수가능</h3>
      <h3>문자열 추출: str.substring(2, 4); // 2, 4 사이 3, 음수는 0</h3>
      <h3>문자열 추출: str.substr(-2, 4); // -2부터 4개</h3>

      <h3>str.codePointAt(value); value위치의 문자에 대한 코드를 반환</h3>
      <h3>str.fromCodePoint(value); value에 대한 문자를 반환</h3>
      <pre>// 90을 16진수로 변환하면 5a입니다.<br/>
  alert( '\u005a' ); // Z</pre>

      <h3>문자열 비교: str.localeCompare(str2);</h3>
      <h3>서로게이트 쌍</h3>
