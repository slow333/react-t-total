...
for (let num of range) {
  alert(num); // 1, then 2, 3, 4, 5
      ...
      <h3>Array.from</h3>
      <p>Array.from을 통해서 이터러블이나 유사배열을 받아 진짜 array를 만들어줍니다.</p>
...
for(let value of Object.values(obj) { ... }`}</pre>
    ...
    <h3>객체 변환하기</h3>
<pre>{`Object.entries(obj) => .map(([key, value]) => [key, value *2]) => wrap Object.fromEntries`}</pre>
...
date.setDate(0); // 일의 최솟값은 1이므로 0을 입력하면 전 달의 마지막 날을 설정한 것과 같은 효과`}</pre>
...
let end = Date.now(); // done
end - start 밀리초; // Date 객체가 아닌 숫자끼리 차감함`}</pre>
    ...
    <pre>{`let ms = Date.parse('2012-01-26T13:51:50.417-07:00'); // 1327611110417  (타임스탬프)
let date = new Date( Date.parse('2012-01-26T13:51:50.417-07:00') );`}</pre>
