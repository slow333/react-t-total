...
/*   form.addEventListener('focus', () => form.classList.add('focused'),  true);
  ...
  return (
    <div>
      <div className="for-space" id='dom-focus-blur'></div>
      ...
      <h5>blur 핸들러에선 필드에 이메일이 잘 입력되었는지 확인하고 잘 입력되지 않은 경우엔 에러를 보여줍니다.</h5>
      <h5>focus 핸들러에선 에러 메시지를 숨깁니다(이메일 재확인은 blur 핸들러에서 합니다).</h5>
...
input.onfocus = function() {
  if (this.classList.contains('invalid')) {
    ...
    <h3>form에서 focus는 버블링되지 않음</h3>
    <h5>캡처링 단계에서 focus, blur를 적용 필요(이벤트리스너에 새번째 인수로 true를 줌)</h5>
    <form id="form" style={{display: "inline-block"}}>
      <input type="text" name="surname" value="성"/>
      <input type="text" name="name" value="이름"/>    </form>

    <h5>또는 버블링되는 focusin, focusout을 적용</h5>
