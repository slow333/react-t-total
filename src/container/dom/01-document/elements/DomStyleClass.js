...
function DomStyleClass() {
  ...
  return (
    <div>
      <div className="for-space" id='dom-style-class'></div>
      <h1>스타일과 클래스</h1>
      <h4>가능하면 스타일 설정보다는 클래스로 설정</h4>

      <h3>classList</h3>
      <h4>elem.classList.add/remove('classname');</h4>
      <h4>elem.classList.toggle('classname');</h4>
      <h4>elem.classList.contains('classname');</h4>
    <pre>{`document.body.style.display = "none"; // hide
setTimeout(() => document.body.style.display = "", 1000); // 1초 후 다시 원래 상태로 돌아옵니다.`}</pre>

      <h3>style.cssText</h3>
      <h4>문자열을 사용해 전체 스타일을 설정</h4>
...
또는 div.setAttribute('style', 'color:red...')`}</pre>
    ...
    <h3>getComputedStyle; 최종 결정값을 반환함</h3>
