 ...
 */
  ...
  return (
    <div>
      <div className="for-space" id='dom-position'></div>
      <h1>좌표</h1>
      <h3>마우스 클릭이 발생한 곳의 위치: event.clientX/pageX, ...</h3>
      <h4>창기준: position:fixed, clientX/clinetY</h4>
      <h4>문서기준: position:absolute, pageX/pageY</h4>
      <img src="/images/page-client-position.png" alt="pageXY, clientXY" width='600' />

    <h3>getBoundingClientRec()로 요소 좌표 얻기</h3>
    <h4>현재 위치한 요소의 좌표 위치를 반환</h4>
    <img src="/images/getBoundingClientRec.png" alt="getBoundingClinetRec()" width='400' />
    <h4>left = x, top = y /  right = x + width, bottom = y + height</h4>

    <h3>elementFromPoint(x, y)</h3>
    <h4>document.elementFromPoint(x, y)을 호출하면 창 기준 좌표 (x, y)에서 가장 가까운 중첩 요소를 반환</h4>
    <h4>예; 창 정중앙에 있는 요소의 태크를 출력</h4>
...
alert(elem.tagName);`}</pre>
    <h4 id="coords-show-mark">나타나는 테크의 위치를 고정하기</h4>
...
setTimeout(() => message.remove(), 5000);</pre>
    <h4>나타나는 메시지의 위치 고정, 스크롤을 계산해서...</h4>
